import React, { useState, useEffect } from "react"; // 1. استوردنا useState و useEffect
import { Link } from "react-router-dom";
import { FaSearch, FaShoppingCart } from "react-icons/fa";

import darkLogo from "../../assets/website/dark.png";
import lightLogo from "../../assets/website/light.png";
...
function Navbar() {
    // 2. عملنا state للـ theme وجبنالها من localStorage عشان تفضل محفوظة
    const [theme, setTheme] = useState(localStorage.getItem("theme") || "light");

    // 3. دالة عشان نغير الـ theme
    const toggleTheme = () => {
        setTheme((prevTheme) => (prevTheme === "light" ? "dark" : "light"));
    };

    // 4. تأثير بيتنفذ كل ما الـ theme يتغير
    useEffect(() => {
        if (theme === "dark") {
                ...
                <div className="container">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            {/* Dark / Light Toggle */}
                            <button onClick={toggleTheme} className="cursor-pointer"> {/* 5. ضفنا الـ onClick */}
                                <img src={lightLogo} alt="theme" className="w-8 dark:hidden" />
                                <img src={darkLogo} alt="theme" className="w-8 hidden dark:block" />
                            </button>
